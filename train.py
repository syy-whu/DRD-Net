...
parser = argparse.ArgumentParser(description="DRDNet_train")
parser.add_argument("--preprocess", type=bool, default=True, help='run prepare_data or not')
parser.add_argument("--batch_size", type=int, default=8, help="Training batch size")
parser.add_argument("--epochs", type=int, default=150, help="Number of training epochs")
parser.add_argument("--milestone", type=int, default=[30,50,80], help="When to decay learning rate")
parser.add_argument("--lr", type=float, default=1e-3, help="initial learning rate")
parser.add_argument("--save_path", type=str, default="logs/Rain200L", help='path to save models and log files')
parser.add_argument("--save_freq",type=int,default=1,help='save intermediate model')
parser.add_argument("--data_path",type=str, default="datasets/Rain200L",help='path to training data')
...
def main():
    ...
    # Build model
    model = DRDNetwork(is_train=True)
    print_network(model)
    ...
    for epoch in range(initial_epoch, opt.epochs):
        ...
        for i, (input_train, target_train) in enumerate(loader_train, 0):
            ...
            out1, out_train = model(input_train)
            out1_loss = l1loss(out1, target_train)
            out_loss = l1loss(out_train, target_train)
            loss = 0.1 * out1_loss + out_loss
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.data_path.find('Rain200H') != -1:
            print(opt.data_path.find('Rain200H'))
            prepare_data_Rain200H(data_path=opt.data_path, patch_size=100, stride=80)
        elif opt.data_path.find('Rain200L') != -1:
            prepare_data_Rain200L(data_path=opt.data_path, patch_size=100, stride=80)
